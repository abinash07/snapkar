<?php
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		if(isset($params['auth_key']) && isset($params['city_id'])){
			$auth_key = mysqli_real_escape_string($con,$params['auth_key']);
			$auth_key = htmlentities($auth_key);

            $city_id = mysqli_real_escape_string($con,$params['city_id']);
			$city_id = htmlentities($city_id);
            ...
            if(mysqli_num_rows($run)){
	            ...
	            $query1 = "SELECT * FROM sk_sub_city_master WHERE status = 1 and city_id = $city_id";
