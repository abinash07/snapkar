<?php
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		if(isset($params['auth_key']) && isset($params['state_id'])){
			$auth_key = mysqli_real_escape_string($con,$params['auth_key']);
			$auth_key = htmlentities($auth_key);

            $state_id = mysqli_real_escape_string($con,$params['state_id']);
            ...
            if(mysqli_num_rows($run)){
	            ...
	            $query1 = "SELECT * FROM sk_city_master WHERE status = 1 and state_id = $state_id";
